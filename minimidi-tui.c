...
// layout - must they really be static? Ask Claude
static const int TOP_BAR_HEIGHT = 3;
static const int BOTT_BAR_HEIGHT = 3;
static const int RIGHT_PANEL_WIDTH = 12;
...
/************************************************
 *                                              *
 *  -= MM_TUI_TrackComponent private methods =- *
 *                                              *      
 ************************************************/
int _tui_TrackComponent_init( MM_TUI_TrackComponent *self, WINDOW *parent, MM_Project *project ){

    log_debug("Entering _tui_TrackComponent_init");

    MM_TUI_TerminalCoords _parent_size;
    getmaxyx(parent, _parent_size.lines, _parent_size.cols);

    self->box_size.cols = _parent_size.cols;
    self->box_size.lines = _parent_size.lines - TOP_BAR_HEIGHT - BOTT_BAR_HEIGHT;
    self->box_position.cols = 0;
    self->box_position.lines = TOP_BAR_HEIGHT + 1;

    self->win = derwin(parent, 
        self->box_size.lines,
        self->box_size.cols,
        self->box_position.lines,
        self->box_position.cols );

    return 0;
}

int _tui_TrackComponent_destroy( MM_TUI_TrackComponent *self ){
    return delwin(self->win);
}

int _tui_TrackComponent_render( MM_TUI_TrackComponent *self ){
    box(self->win, 0, 0);
    return mvwprintw(self->win, 0, 2, "Track Component");
}

int _tui_TrackComponent_update( MM_TUI_TrackComponent *self, double t_s ){
    return 0;
}

int _tui_TrackComponent_load_track( MM_TUI_TrackComponent *self, MM_Track *t ){
    return 0;
}
...
/**
 * MM_TUI_GeneralInfoComponent private methods
 */
int _tui_GeneralInfoComponent_init( MM_TUI_GeneralInfoComponent *self,  WINDOW *parent, MM_Project *project ){
    MM_TUI_TerminalCoords _parent_size;
    getmaxyx(parent, _parent_size.lines, _parent_size.cols);
    
    self->box_size.cols = _parent_size.cols;
    self->box_size.lines = TOP_BAR_HEIGHT;
    self->box_position.cols = 0;
    self->box_position.lines = 0;
...
int _tui_GeneralInfoComponent_render( MM_TUI_GeneralInfoComponent *self){
    box(self->win, 0, 0);
    return mvwprintw(self->win, 0, 2, "General Info Component");
}

int _tui_GeneralInfoComponent_destroy( MM_TUI_GeneralInfoComponent *self ){
    return delwin(self->win);
}


/**
 * MM_TUI_TrackPropertiesComponent private methods
 */
int _tui_TrackPropertiesComponent_init( MM_TUI_TrackPropertiesComponent *self,  WINDOW *parent, MM_Project *project ){
    MM_TUI_TerminalCoords _parent_size;
    getmaxyx(parent, _parent_size.lines, _parent_size.cols);
    
    self->box_size.cols = RIGHT_PANEL_WIDTH;
    self->box_size.lines =  _parent_size.lines - TOP_BAR_HEIGHT - BOTT_BAR_HEIGHT;
    self->box_position.cols = 0;
    self->box_position.lines = _parent_size.cols - RIGHT_PANEL_WIDTH;
...
int _tui_TrackPropertiesComponent_destroy( MM_TUI_TrackPropertiesComponent *self ){
    if (del_panel(self->pnl) != 0){
        return 1;
    }
    
    return delwin(self->win);
}
int _tui_TrackPropertiesComponent_render( MM_TUI_TrackPropertiesComponent *self ){
    if (self->is_shown){
        box(self->win, 0, 0);
        return mvwprintw(self->win, 0, 0, "TrackPropertiesComponent");
        
    }
    return 0;
}
int _tui_TUI_TrackPropertiesComponent_update( MM_TUI_TrackPropertiesComponent *self, double t_s ){
    if (self->is_shown){
        show_panel(self->pnl);
    }else {
        hide_panel(self->pnl);
...
int _tui_TUI_TrackPropertiesComponent_load_track( MM_TUI_GeneralInfoComponent *self, MM_Track *t ){
    return 0;
}

/**
 * MM_TUI_TimeInfoComponent private methods
 */
int _tui_TimeInfoComponent_init( MM_TUI_TimeInfoComponent *self,  WINDOW *parent, MM_Project *project ){
    MM_TUI_TerminalCoords _parent_size;
    getmaxyx(parent, _parent_size.lines, _parent_size.cols);
    
    self->box_size.cols = _parent_size.cols;
    self->box_size.lines = BOTT_BAR_HEIGHT;
    self->box_position.cols = 0;
    self->box_position.lines = _parent_size.lines - BOTT_BAR_HEIGHT - 1;
...
int _tui_TimeInfoComponent_render( MM_TUI_TimeInfoComponent *self){
    box(self->win, 0, 0);
    return mvwprintw(self->win, 0, 2, "TimeInfoComponent");
}

int _tui_TimeInfoComponent_destroy( MM_TUI_TimeInfoComponent *self){
    return delwin(self->win);
}

/**
 * @brief Calculate layout from MM_TUI's outer size
 * 
 */
int _tui_calculate_layout( MM_TUI *self ){

    getmaxyx(stdscr, self->box.lines, self->box.cols);
    self->general_info_component.box_size.cols = self->box.cols;
    self->time_info_component.box_size.cols = self->box.cols;

    self->track_component.box_size.lines = self->box.lines
        - self->general_info_component.box_size.lines
        - self->time_info_component.box_size.lines;
    self->track_component.box_size.cols = self->box.cols;

    self->track_properties_component.box_size.lines = self->track_component.box_size.lines;

    return 0;
}

int _tui_handle_input( MM_TUI *self )
{
    int key = getch();
...
int _tui_render(MM_TUI *self){

    _tui_GeneralInfoComponent_render(&(self->general_info_component));
    _tui_TrackComponent_render(&(self->track_component));
    _tui_TimeInfoComponent_render(&(self->time_info_component));
    touchwin(stdscr);
    wrefresh(stdscr);
    return 0;
}
...
int MM_TUI_init( MM_TUI *self, MM_Project *project, MM_Ring_Buffer *cmd_queue, MM_AudioEngine *audio_engine){
    ...
    */
    ...
    getmaxyx(stdscr, self->box.lines, self->box.cols);
    log_debug("Aquired terminal Window size = (%i lines, %i cols)", self->box.lines, self->box.lines);

    /**
    * init Sub Components
    * 
    */
    _tui_TrackComponent_init(&(self->track_component), stdscr, self->project );
    _tui_GeneralInfoComponent_init(&(self->general_info_component), stdscr , self->project );
    _tui_TrackPropertiesComponent_init(&(self->track_properties_component), stdscr , self->project );
    _tui_TimeInfoComponent_init(&(self->time_info_component), stdscr, self->project );
    
    _tui_calculate_layout( self );
...
int MM_TUI_step(MM_TUI *self){
    _tui_handle_input(self);
    _tui_render(self);
}

int MM_TUI_destroy( MM_TUI *self ){
    // DESTROY COMPONENTS
    if (_tui_TrackComponent_destroy(&(self->track_component))
        + _tui_GeneralInfoComponent_destroy(&(self->general_info_component))
        + _tui_TimeInfoComponent_destroy(&(self->time_info_component)) ){
